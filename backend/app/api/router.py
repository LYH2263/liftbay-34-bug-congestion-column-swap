...
def congestion(db: Session = Depends(get_db)):
    rows = db.scalars(
        select(CallTicket).where(CallTicket.status.in_(("waiting", "assigned")))
    ).all()
    counts = congestion_split_by_floor(
        [
            CallRequest(c.id, c.floor, c.direction, c.passengers, c.status)
            for c in rows
        ]
    )
    return [
        CongestionFloor(
            floor=f, waiting=s.waiting, assigned=s.assigned, total=s.total
        )
        for f, s in sorted(counts.items(), key=lambda x: (-x[1].total, x[0]))
    ]
